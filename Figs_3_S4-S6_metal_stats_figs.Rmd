...
has stats & figs for figures 3, S4 thru S5
...
this is the figure for the metals normalized to Phos (Fig S5)
```{r}
#######################
# make the figure

# rename dataframe
molP <- read_excel("~/Desktop/PhD/chapters/chapter 4 - temp treatments/for_coauthors_v1/data_code/metallome_molP.xlsx",
      sheet = "formanova")
...
# melt the data
# the order of the measured variables is important for facet wrapping later
melt2 <- melt(molP, id=c("Species", "Ironconc", "Temp", "TreatmentID"), measure.vars=c("Iron","Zinc","Manganese", "Copper", "Cobalt","Nickel"))
melt2 <- as.data.frame(melt2)
...
melt2$TreatmentID <- as.factor(melt2$TreatmentID)
melt2$variable <- as.factor(melt2$variable)
melt2$variable <- factor(melt2$variable, levels = c("Iron", "Manganese", "Zinc", "Cobalt", "Copper", "Nickel"))
...
# make the bar graph
bbar1 <- ggplot(data=stats, aes(x = Species, y=avg, fill=Temp)) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single")) +
  geom_errorbar(aes(ymin=avg-stdev, ymax=avg+stdev), width=0.9, colour="black", position=position_dodge2(width = 0.9, preserve = "single")) +
  theme_bw() +
  theme(axis.text.x = element_text(angle=20, vjust=1, hjust=1), axis.title.x = element_blank()) +
  ...
  theme(legend.text = element_text(size=14)) + 
  theme(legend.title = element_text(size=14, face = "bold")) +
  labs(y=expression(bold(mmol~metal~mol~P^-1)), x="Species") +
  theme(legend.position = "bottom")  +
  theme(strip.text = element_text(size=12, face = "bold",margin = margin(.15, .15, .15, .15, "cm"))) +
  scale_fill_manual(values = cols, name = "Temperature", breaks = c("26", "28", "30"), labels=c("26 °C", "28 °C", "30 °C")) +
  scale_x_discrete()  
...
ggsave("Figx_metal_molP.pdf", bbar1, width = 6, height = 10, units = "in")
```

multivariate figures and stats
```{r}
