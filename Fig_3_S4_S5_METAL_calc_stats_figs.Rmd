...
CODE GRAVEYARD
###############
multiply by vol per cell
```{r eval=FALSE, include=FALSE}
# read in the volpercell
vpc <- read_excel("~/Desktop/PhD/chapters/chapter 4 - temp treatments/for_coauthors_v1/data_code/cell_density.xlsx", sheet ="vol")
# join them
both <- merge(dat1, vpc, by = "sampleID")

# create variables
both$Mn_CV <- (both$Manganese*both$VolPerCell)
both$Fe_CV <- (both$Iron*both$VolPerCell)
both$Co_CV <- (both$Cobalt*both$VolPerCell)
both$Ni_CV <- (both$Nickel*both$VolPerCell)
both$Cu_CV <- (both$Copper*both$VolPerCell)
both$Zn_CV <- (both$Zinc*both$VolPerCell)
both$P_CV <- (both$Phosphorus*both$VolPerCell)

# melt the data
meltvpc <- reshape2::melt(both, id.vars = c("sampleID", "Species", "Ironconc", "Temp"), measure.vars = c("Mn_CV", "Fe_CV", "Co_CV", "Ni_CV", "Cu_CV", "Zn_CV", "P_CV"))

# spits out figs for each ratio
metalz_to_vpc <- function(mycolumn){
  for (i in 1:length(unique(mycolumn))) {
    mycolumn2 <- as.vector(unique(mycolumn))
    fdata <- filter(meltvpc, mycolumn %in% unique(mycolumn)[i])
    myplot <- ggplot(data=fdata, aes(x=Ironconc, y=value, fill=Species, color= Species)) +
  geom_boxplot(alpha = 0.5, varwidth = FALSE, position = position_dodge(1, preserve = "single")) +
  theme_bw() +
  labs(y="(pg/cell)(vol/cell)", x=element_blank(), title = element_blank()) +
  theme(panel.grid = element_blank(),
        strip.text.x = element_text(face = "bold"),
        strip.text.y = element_text(face = "bold"),
        legend.title = element_text(face = "bold"),
...
metalz_to_vpc(meltvpc$variable)
