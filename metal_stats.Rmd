---
title: "FigS2_thru_S3_metallome_um3"
author: "Hannah Reich"
date: "July 15, 2019"
output: html_document
---

script updated 04/22/2020. removed P & C normalizations because P & C fluctuate with temperature. OG file = "~/Desktop/PhD/chapters/chapter 4 - temp treatments/for_coauthors_v1/data_code/code/FigS2_thru_S3_metallome_um3"


```{r}
# figures S2 & S3
# um3 per cell normalized metallome data
...
melt2 <- melt(um3, id=c("Species", "Ironconc", "Temp", "TreatmentID"), measure.vars=c("Iron","Zinc","Manganese", "Copper", "Cobalt","Nickel", "Phosphorus"))
...
melt2$variable <- factor(melt2$variable, levels = c("Cobalt", "Copper", "Iron", "Manganese", "Nickel", "Zinc", "Phosphorus"))
...
bbar <- ggplot(data=stats, aes(x = Species, y=avg, fill=Temp)) +
  ...
  theme(axis.text.x = element_text(angle=25, vjust=1, hjust=1), axis.title.x = element_blank()) +
...
ggsave("FigS2_metallome_um3.pdf", bbar, width = 6, height = 10, units = "in")
...
# make the plot
bmin <- ggplot(newdata1) + 
  geom_point(aes(newdata1$lda.LD1, newdata1$lda.LD2, 
                 color = newdata1$Temp, 
                 shape = newdata1$Ironconc), 
                 size = 3.5) + 
  theme_bw() +
  labs(title = "Breviolum minutum",
       x = "LD1",
       y = "LD2") +
  ...
  theme(legend.title = element_text(size=10, face="bold"), legend.title.align = 0) +
  stat_ellipse(geom = "polygon", type = "t", alpha = 0.125, show.legend = FALSE,
               aes(newdata1$lda.LD1, newdata1$lda.LD2, fill = newdata1$Temp)) +  
               scale_fill_manual(values = cols) + theme_cowplot()
bmin

#save_plot("FigS3A_bmin.pdf", bmin, base_aspect_ratio = 1.6)
...
# make the plot
bpsyg <- ggplot(newdata) + 
  geom_point(aes(newdata$lda.LD1, newdata$lda.LD2, 
                 color = newdata$Temp, 
                 shape = newdata$Ironconc), 
                 size = 3.5) + 
  theme_bw() +
  labs(title = "Breviolum psygmophilum",
       x = "LD1",
       y = "LD2") +
  scale_color_manual(values = cols, breaks = c("26", "28", "30"), labels=c("26°C", "28°C", "30°C"), name = "Temperature") +
  scale_shape_manual(values = c(21,22,24),
  ...
  theme(legend.title = element_text(size=10, face="bold"), legend.title.align = 0) +
  stat_ellipse(geom = "polygon", type = "t", alpha = 0.125, show.legend = FALSE,
               aes(newdata$lda.LD1, newdata$lda.LD2, fill = newdata$Temp)) +  
               scale_fill_manual(values = cols) + theme_cowplot()
bpsyg

bmin

#save_plot("FigS3B_bpsyg.pdf", bpsyg, base_aspect_ratio = 1.6)

# plot grid them together
legend <- get_legend(bpsyg)

brev <- plot_grid(bmin + theme(legend.position = "none"), bpsyg + theme(legend.position = "none"), legend, nrow=1)
brev

#save_plot("~/Desktop/PhD/chapters/chapter 4 - temp treatments/for_coauthors_v1/figs/figs_v4_01212020/Fig4A_um3_multivar.pdf",brev, nrow=1, ncol=2, base_aspect_ratio = 1.2)
...
both
save_plot("~/Desktop/PhD/chapters/chapter 4 - temp treatments/for_coauthors_v1/figs/figs_v4_01212020/Fig4_um3_multivar.pdf",both, base_aspect_ratio = 1.9)

```


part 4: normalization of POC and PON to cell vol
```{r}
# this part of the script normalizes all of the metals to Particulate Organic Carbon (POC).
# calculation info for the POC and Particulate Organic Nitrogen (PON) is in the excel file "poc_pon"
...
# read in the data
dat <- read_excel("~/Desktop/PhD/chapters/chapter 4 - temp treatments/for_coauthors_v1/data_code/poc_pon.xlsx", sheet = "pocpon_forR")
dat <- as.data.frame(dat)

# normalize the POC and PON content to vol per cell
...
volused$cellvol_pocpon <- (volused$TotalVol*volused$mLPOCPON)

# merge the data
all <- merge(dat, volused, by = "sampleID")
all <- as.data.frame(all)

# now calculate C and N per uM3 cell
all$C_Cell <- ((all$POC_uM/all$TotalVol)*1000)
all$N_Cell <- ((all$PON_uM/all$TotalVol)*1000)

# write out file with just sample info and N and C per cell
all1 <- all[,c(1:5,16:17)]
write_csv(all1, "POCPON_cell.csv")
